...
//#include "ExactPhraseScorer.hpp"
//#include "SloppyPhraseScorer.hpp"
#include "../../searcher/QueryResolver.hpp"
//#include "unicode/ustdio.h"
#include "unicode/ustring.h"

U_STRING_DECL(cStrClassName, "PhraseQuery", 11);
CL_NS_DEF(search)
void PhraseQuery::InitStringDecl() {
	U_STRING_INIT(cStrClassName, "PhraseQuery", 11);
}
CL_NS_END
...
bool PhraseQuery::equals(CL_NS(search)::Query *other) const
{
	...
	PhraseQuery* pq = static_cast<PhraseQuery *>(other);
...
const UChar *PhraseQuery::getClassName()
{
	return cStrClassName;
}

const UChar *PhraseQuery::getQueryName() const
{
...
void PhraseQuery::add(Term* term, int32_t position)
{
	...
	if (terms.size() == 0)
		...
		if (_tcscmp(term->field(), field) != 0) {
			...
			_CLTHROWT(CL_ERR_IllegalArgument, "All phrase terms must be in the same field"); //buf);
		}
	}
...
void PhraseQuery::getPositions(Array<int32_t>& result) const
{
	...
	for (int32_t i = 0; i < (int32_t)result.length; i++) {
...
UChar *PhraseQuery::toString(const UChar *f) const
{
	...
	if (slop != 0) {
		buffer.append(_T("~"));
		buffer.appendFloat((float_t)slop, 0);
	}
...
Atoll::EntrySet* PhraseQuery::Search(Searcher *inSearcher)
{
	Term *t;
	//int32_t p;
	Query *query;
	...
	for (uint32_t i = 0 ; i < size; i++) {
		...
		entrySet = query->Search(inSearcher);
		delete query;
...
UChar *PhraseQuery::PhraseWeight::toString()
{
...
Query* PhraseQuery::PhraseWeight::getQuery()
{
	return _this;
}
float_t PhraseQuery::PhraseWeight::getValue()
{
	return value;
}
...
{
	UChar descbuf[LUCENE_SEARCH_EXPLANATION_DESC_LEN+1];
	UChar *tmp;
