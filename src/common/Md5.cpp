...
public:
	...
	MD5              (const unsigned char *string, unsigned int length); // digest string, finalize
	MD5              (const std::string& text); // digest string, finalize
	MD5              (std::istream& stream);       // digest stream, finalize
	MD5              (FILE *file);            // digest file, finalize
	MD5              (std::ifstream& stream);      // digest stream, finalize
...
void Common::Md5String(std::string &outMd5, const std::string &inStr)
{
	MD5 md5(inStr);
	outMd5 = md5.hexdigest();
}
//------------------------------------------------------------------------------

void Common::Md5BinaryBuffer(std::string &outMd5, const BinaryBuffer &inBinaryBuffer)
{
	const unsigned char *buf = (const unsigned char*)inBinaryBuffer.getBuffer();
	unsigned int length = inBinaryBuffer.getOccupancy();
...
void Common::Md5File(std::string &outMd5, const std::string &inFullFileName)
{
  FILE *f = fopen(inFullFileName.c_str(), "rb");
	if (f == NULL) {
		outMd5.clear();
		gLog.log(Common::eTypLogError, "Err > Unable to open file: %s", inFullFileName.c_str());
		return;
	}
...
void MD5::update (uint1 *input, uint4 input_length)
{
  ...
  if (finalized) {
		gLog.log(Common::eTypLogError, "Err > Md5 update cannot be finalized");
		return;
	}
...
void MD5::finalize()
{
  ...
  if (finalized) {
		gLog.log(Common::eTypLogError, "Err > Md5 finalize cannot be already finalized");
		return;
	}
...
MD5::MD5(const unsigned char *string, unsigned int length)
{
  init();  // must be called be all constructors
  update((unsigned char *)string, length);
  finalize ();
}
...
{
  if (!finalized) {
		gLog.log(Common::eTypLogError, "Err > Md5 hexdigest cannot be done if the digest is not finalized");
		return "";
	}
...
void MD5::transform (uint1 block[64])
{
  ...
  if (finalized) {
		gLog.log(Common::eTypLogError, "Err > Md5 transform cannot be finalized");
		return;
	}
